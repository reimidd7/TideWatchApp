...
// ============================================================================
// SETTINGS MODAL - Add this to your app.js
// ============================================================================
...
// Initialize settings when DOM loads (add to your existing DOMContentLoaded)
function initSettings() {
    setupSettingsEventListeners();
    console.log('⚙️ Settings initialized');
}

// Call this in your existing setupEventListeners function or DOMContentLoaded
function setupSettingsEventListeners() {
    ...
    // WiFi controls
    document.getElementById('wifi-refresh-btn')?.addEventListener('click', loadWifiStatus);
    document.getElementById('wifi-scan-btn')?.addEventListener('click', scanWifiNetworks);
    
    // WiFi password dialog
    document.getElementById('wifi-cancel-connect')?.addEventListener('click', hidePasswordDialog);
    document.getElementById('wifi-confirm-connect')?.addEventListener('click', confirmWifiConnect);
    document.getElementById('wifi-toggle-password')?.addEventListener('click', togglePasswordVisibility);
    ...
    // System controls
    document.getElementById('btn-restart-kiosk')?.addEventListener('click', restartKiosk);
    document.getElementById('btn-reboot')?.addEventListener('click', rebootSystem);
...
function openSettingsModal() {
    ...
    if (modal) {
        ...
        loadTabData(settingsState.currentTab);
    }
}
...
async function loadWifiStatus() {
    ...
    container.innerHTML = `<div class="wifi-status-loading"><i class="fas fa-spinner fa-spin"></i> Checking...</div>`;
    ...
    try {
        ...
        if (result.status === 'ok' && result.data.connected) {
            ...
            const signalLevel = Math.ceil((parseInt(data.signal) || 0) / 25);
            
            container.innerHTML = `
                <div class="wifi-connected">
                    <div class="wifi-connected-icon"><i class="fas fa-wifi"></i></div>
                    <div class="wifi-connected-details">
                        <div class="wifi-ssid">${data.ssid}</div>
                        <div class="wifi-ip">${data.ip_address || 'Getting IP...'}</div>
                    </div>
                    <div class="wifi-signal-bar">
                        ${[1,2,3,4].map(i => `<span style="height: ${i * 6}px" class="${i <= signalLevel ? 'active' : ''}"></span>`).join('')}
                    </div>
                </div>
            `;
        } else {
        ...
        container.innerHTML = `<div class="wifi-disconnected"><i class="fas fa-exclamation-triangle"></i> Error checking status</div>`;
    }
}

async function scanWifiNetworks() {
    const container = document.getElementById('wifi-networks-list');
    container.innerHTML = `<div class="wifi-status-loading"><i class="fas fa-spinner fa-spin"></i> Scanning...</div>`;
    ...
    try {
        ...
        if (result.status === 'ok' && result.data.length > 0) {
            settingsState.wifiNetworks = result.data;
            
            container.innerHTML = result.data.map(network => `
                <div class="wifi-network-item ${network.connected ? 'connected' : ''}" 
                     data-ssid="${network.ssid}" 
                     data-security="${network.security}">
                    <div class="wifi-network-icon">
                        <i class="fas ${network.security !== 'Open' ? 'fa-lock' : 'fa-wifi'}"></i>
                    </div>
                    <div class="wifi-network-info">
                        <div class="wifi-network-name">${network.ssid}</div>
                        <div class="wifi-network-security">${network.security}</div>
                    </div>
                    <div class="wifi-network-signal">${network.signal}%</div>
                </div>
            `).join('');
            
            // Add click handlers
            container.querySelectorAll('.wifi-network-item').forEach(item => {
                item.addEventListener('click', () => handleNetworkClick(item));
            });
        } else {
            container.innerHTML = `<div class="wifi-status-loading"><i class="fas fa-wifi-slash"></i> No networks found</div>`;
        }
    } catch (error) {
        console.error('Error scanning WiFi:', error);
        container.innerHTML = `<div class="wifi-status-loading"><i class="fas fa-exclamation-triangle"></i> Scan failed</div>`;
    }
}

function handleNetworkClick(item) {
    const ssid = item.dataset.ssid;
    const security = item.dataset.security;
    
    if (item.classList.contains('connected')) {
        // Already connected - maybe offer to disconnect?
        return;
    }
    
    settingsState.selectedNetwork = { ssid, security };
...
function togglePasswordVisibility() {
    ...
    const icon = document.querySelector('#wifi-toggle-password i');
...
async function confirmWifiConnect() {
    ...
    const ssid = settingsState.selectedNetwork.ssid;
...
async function connectToWifi(ssid, password) {
    const container = document.getElementById('wifi-networks-list');
    const networkItem = container.querySelector(`[data-ssid="${ssid}"]`);
    
    if (networkItem) {
        networkItem.innerHTML = `
            <div class="wifi-network-icon"><i class="fas fa-spinner fa-spin"></i></div>
            <div class="wifi-network-info">
                <div class="wifi-network-name">${ssid}</div>
                <div class="wifi-network-security">Connecting...</div>
            </div>
        `;
    }
    ...
    try {
        ...
        if (result.data?.success) {
            ...
            console.error('❌ WiFi connection failed:', result.data?.message);
            alert(`Failed to connect: ${result.data?.message || 'Unknown error'}`);
            await scanWifiNetworks();
        }
    } catch (error) {
...
async function loadSystemStatus() {
    try {
        ...
        if (result.status === 'ok') {
            const data = result.data;
            settingsState.systemStatus = data;
            
            updateElement('sys-cpu', data.cpu_usage ? `${data.cpu_usage}%` : '--%');
            updateElement('sys-memory', data.memory?.percent ? `${data.memory.percent}%` : '--%');
            updateElement('sys-temp', data.temperature ? `${data.temperature}°C` : '--°C');
            updateElement('sys-disk', data.disk?.percent || '--%');
            updateElement('sys-uptime', data.uptime || '--');
        }
    } catch (error) {
...
async function restartKiosk() {
    if (!confirm('Restart the kiosk browser? The display will briefly go blank.')) return;
    
    try {
        const response = await fetch(`${API_BASE}/api/system/restart-kiosk`, { method: 'POST' });
        const result = await response.json();
        
        if (result.data?.success) {
            // The page will reload when kiosk restarts
            document.body.innerHTML = `
                <div style="display: flex; align-items: center; justify-content: center; 
                            height: 100vh; background: var(--color-bg); color: var(--color-text);">
                    <div style="text-align: center;">
                        <i class="fas fa-sync fa-spin" style="font-size: 3rem; margin-bottom: 20px;"></i>
                        <h2>Restarting Kiosk...</h2>
                    </div>
                </div>
            `;
        } else {
            alert('Failed to restart kiosk: ' + (result.data?.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error restarting kiosk:', error);
        alert('Error restarting kiosk');
    }
}

async function rebootSystem() {
    if (!confirm('⚠️ REBOOT the Raspberry Pi?\n\nThis will take about a minute.')) return;
    if (!confirm('Are you sure? The system will go offline temporarily.')) return;
    
    try {
        const response = await fetch(`${API_BASE}/api/system/reboot`, { method: 'POST' });
        const result = await response.json();
        ...
        if (result.data?.success) {
            document.body.innerHTML = `
                <div style="display: flex; align-items: center; justify-content: center; 
                            height: 100vh; background: var(--color-bg); color: var(--color-text);">
                    <div style="text-align: center;">
                        <i class="fas fa-power-off" style="font-size: 3rem; margin-bottom: 20px; color: #D95E5E;"></i>
                        <h2>Rebooting...</h2>
                        <p>Please wait about 60 seconds</p>
                    </div>
                </div>
            `;
        } else {
            alert('Failed to reboot: ' + (result.data?.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error rebooting system:', error);
        alert('Error initiating reboot');
    }
}
...
async function checkForUpdates() {
    ...
    const updateInfo = document.getElementById('update-available');
    const pendingSection = document.getElementById('pending-commits');
    ...
    try {
        ...
        if (result.status === 'ok') {
            const data = result.data;
            settingsState.updateInfo = data;
            
            updateElement('current-version', data.current_version || 'unknown');
            
            if (data.updates_available && data.pending_commits > 0) {
                updateInfo.innerHTML = `<i class="fas fa-arrow-circle-down"></i> ${data.pending_commits} update(s) available`;
                updateInfo.className = 'update-available has-updates';
                
                // Show commits
                pendingSection.style.display = 'block';
                commitsList.innerHTML = data.commits.map(c => `<li>${c}</li>`).join('');
                
                updateBtn.disabled = false;
            } else {
                updateInfo.innerHTML = '<i class="fas fa-check-circle"></i> System is up to date';
        ...
        updateInfo.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Error checking updates';
        updateInfo.className = 'update-available';
    }
...
async function performUpdate() {
    if (!confirm('Install updates now?\n\nThe application will restart after updating.')) return;
    ...
    try {
        ...
        progressText.textContent = 'Downloading updates...';
        
        const response = await fetch(`${API_BASE}/api/system/update`, { method: 'POST' });
        ...
        if (result.data?.success) {
            ...
            progressText.textContent = 'Update failed: ' + (result.data?.message || 'Unknown error');
            progressFill.style.background = '#D95E5E';
            updateBtn.disabled = false;
        }
    } catch (error) {
        console.error('Error performing update:', error);
        progressText.textContent = 'Update error. Please try again.';
        progressFill.style.background = '#D95E5E';
        updateBtn.disabled = false;
    }
}
